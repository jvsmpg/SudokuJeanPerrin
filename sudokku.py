#fonction qui donne la liste des valeurs que l'in peut mettre à la ligne et colonne de grille
#L est l'ensemble des choix possibles
L = [1,2,3,4,5,6,7,8,9]
#fonction qui affecte la valeur à la position intiquée dans la grille passée en paramètre
...
#fonction qui donne la valeur du coefficient de grille à la position passée en paramètre
def valeur_position(position,grille):
    indice_ligne = int(position / 9)
    indice_colonne = position % 9
    return grille[indice_ligne][indice_colonne]
#fonction qui extrait la ligne de la grille a l'indice passé en paramètre
def extraire_ligne(indice,grille):
    l = []
    for j in range(0, 9):
        l.append(grille[indice][j])
    return l
#fonction qui extrait la colonne de la grille à l'indice passé en paramètre
def extraire_colonne(indice,grille):
    l = []
    for i in range(0,9):
        l.append(grille[i][indice])
    return l
#fonction qui extrait le bloc corrspondant à la ligne et à la colonne passée en paramètre
...
#fonction qui donne la liste des choix possible à la position passée en paramètre
def choix_possible(position,grille):
    ...
    liste = list(liste)  # reconvertir l'ensemble en liste indexée
    return liste

# fonction récursive qui precise si on peut remplir en position suivante à partir du choix fait en position courante
...
#fonction qui complete la grille passée en paramètre
